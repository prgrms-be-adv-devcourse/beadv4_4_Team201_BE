...
class MemberViewSyncListenerTest {
    ...
    private final MemberViewSyncListener listener = new MemberViewSyncListener(repository, eventPublisher);
    ...
    @Test
    @DisplayName("MemberUpdatedEvent (SELLER) 수신 시 view 가 갱신되고 SellerNicknameChangedEvent 가 발행된다")
    void sellerUpdated_updates_view_and_publishes_seller_event() {
        Long memberId = 2L;
        ...
        listener.on(new MemberUpdatedEvent(memberId, "auth0|x", nickname, MemberRole.SELLER));

        assertThat(existing.getNickname()).isEqualTo(nickname);
    ...
    @Test
    @DisplayName("MemberUpdatedEvent (BUYER) 수신 시 view 만 갱신되고 SellerNicknameChangedEvent 는 발행되지 않는다")
    void buyerUpdated_updates_view_only() {
        Long memberId = 3L;
        MemberView existing = new MemberView(memberId, "이전");
        given(repository.findById(memberId)).willReturn(Optional.of(existing));
