        ...
        repository.findByIdempotencyKey(idempotencyKey)
                .ifPresent(history -> {
                    if (payloadHasher.isMatch(history.getPayloadHash(), payloadHasher.calculateHash(payload))) {
                        throw new PolicyException(app.giftify.shared.api.exception.IdempotencyErrorCode.DUPLICATE_REQUEST);
                    } else {
                        throw new PolicyException(app.giftify.shared.api.exception.IdempotencyErrorCode.PAYLOAD_MISMATCH);
                    }
                });
    }
}
