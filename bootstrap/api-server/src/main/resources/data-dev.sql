-- =============================================================================
-- Giftify Development Environment Seed Data (PostgreSQL)
-- =============================================================================
-- 이 파일은 dev 프로파일에서 서버 시작 시 자동 실행됩니다.
-- PostgreSQL + Docker Compose 환경용
-- ON CONFLICT DO NOTHING으로 중복 실행 시에도 안전합니다.
-- =============================================================================
...
VALUES (1, 'qa-seller-giftify@team201.dev', '1Q@W3r4R1q2w3e4r', '멍청한돼지0009', '1975-11-08', 'SELLER', '서울시 종로구',
       ...
       (4, 'admin-giftify@team201.dev', '@675995dC43786719eb76Cd455d3b368', '관리자', '1999-01-01', 'ADMIN', '서울시 강남구',
        '010-0000-0000', 'TEAM201', 'ACTIVE', 'auth0|6981843a226ff0ca1e6a5ae8', NOW(), NOW(), 'SYSTEM',
        'SYSTEM') ON CONFLICT (id) DO NOTHING;

-- PostgreSQL 시퀀스 조정
SELECT setval('members_id_seq', GREATEST((SELECT MAX(id) FROM members), 100), false);
...
VALUES (1, '멍청한돼지0009'),
       (2, '나른한고양이0013'),
       (3, '멍청한고양이2013'),
       (4, '관리자') ON CONFLICT (id) DO NOTHING;
...
VALUES (1, 'auth0|6981838d48f8397cae06ddb0', '멍청한돼지0009'),
       ...
       (4, 'auth0|6981843a226ff0ca1e6a5ae8', '관리자') ON CONFLICT (id) DO NOTHING;
...
VALUES (1, 1, 100000.00, 0, NOW(), NOW(), 'SYSTEM', 'SYSTEM'),
       ...
       (4, 4, 0.00, 0, NOW(), NOW(), 'SYSTEM', 'SYSTEM') ON CONFLICT (id) DO NOTHING;

SELECT setval('wallet_id_seq', GREATEST((SELECT MAX(id) FROM wallet), 100), false);
...
VALUES (1, 1, NOW(), NOW(), 'SYSTEM', 'SYSTEM'),
       ...
       (4, 4, NOW(), NOW(), 'SYSTEM', 'SYSTEM') ON CONFLICT (id) DO NOTHING;

SELECT setval('cart_id_seq', GREATEST((SELECT MAX(id) FROM cart), 100), false);
...
VALUES (1, 1, 'PRIVATE', NOW(), NOW(), 'SYSTEM', 'SYSTEM'),
       ...
       (4, 4, 'PRIVATE', NOW(), NOW(), 'SYSTEM', 'SYSTEM') ON CONFLICT (id) DO NOTHING;

SELECT setval('wishlist_id_seq', GREATEST((SELECT MAX(id) FROM wishlist), 100), false);
...
VALUES
    ...
    (10, 3, '몽블랑 볼펜', '마이스터스튁 클래식 블랙', 520000, 50, 'DRAFT', NOW(), NOW(), 'SYSTEM',
     'SYSTEM') ON CONFLICT (id) DO NOTHING;

SELECT setval('product_id_seq', GREATEST((SELECT MAX(id) FROM product), 100), false);
...
VALUES (1, 1, 1, 'PENDING', NOW(), NOW(), 'SYSTEM', 'SYSTEM'),
       ...
       (3, 2, 3, 'PENDING', NOW(), NOW(), 'SYSTEM', 'SYSTEM') ON CONFLICT (id) DO NOTHING;

SELECT setval('wishlist_item_id_seq', GREATEST((SELECT MAX(id) FROM wishlist_item), 100), false);
...
SELECT setval('order_v2_id_seq', GREATEST((SELECT MAX(id) FROM order_v2), 100), false);
...
SELECT setval('order_item_v2_id_seq', GREATEST((SELECT MAX(id) FROM order_item_v2), 100), false);
